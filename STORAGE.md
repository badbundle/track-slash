...
Do not store raw object bytes in Postgres. Product features such as issue/sprint attachments or image fields should reference `storage_objects.id` from their own tables.

Issue and sprint description attachments are documented in `ATTACHMENTS.md`. Attachment links store only parent/object/audit data; object metadata such as filename, content type, byte size, hash, backend, bucket, and object key remains in `storage_objects`.
